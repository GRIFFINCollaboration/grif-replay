...
// TBragg
// Parameters taken from MSc thesis of Chantal Nobs for the 12C beam test
// https://grsi.wiki.triumf.ca/w/images/1/1b/Nobs-MPhys-Dissertation-cropped.pdf
#define TBRAGG_WINDOW_WIDTH 14  // 140ns for window width
#define TBRAGG_WINDOW_GAP   24  // 240ns gap time between start time of each window
...
int unpack_grif3_event(unsigned *evntbuf, int evlen, Grif_event *ptr, int process_waveforms)
{
  ...
  float tbragg_sum1, tbragg_sum2, tbragg_filter, tbragg_filter_max=0;
  ...
  for(i=0; i<evlen; i++){
    ...
    */
    if( process_waveforms == 1 ){/* + 14->16bit sign extension */
      ...
      // TBragg
      // Waveforms use 300 samples with pre-trigger of 100
      ...
      tbragg_sum1 = tbragg_sum2 = 0;
      tbragg_ptr = (wave_ptr-2-TBRAGG_WINDOW_GAP-TBRAGG_WINDOW_WIDTH-TBRAGG_WINDOW_WIDTH);
      if(tbragg_ptr>=0){
        for(k=0; k<TBRAGG_WINDOW_WIDTH; k++){
          tbragg_window1[tbragg_ptr+k]   = waveform[tbragg_ptr+k];
          tbragg_window2[tbragg_ptr+k]   = waveform[tbragg_ptr+TBRAGG_WINDOW_GAP+k];
          tbragg_sum1 += waveform[tbragg_ptr+k];
          tbragg_sum2 += waveform[tbragg_ptr+TBRAGG_WINDOW_GAP+k];
        }
        tbragg_filter = (tbragg_sum2/TBRAGG_WINDOW_WIDTH) - (tbragg_sum1/TBRAGG_WINDOW_WIDTH);
        if(tbragg_filter > tbragg_filter_max){
          tbragg_filter_max = tbragg_filter;
          //ptr->alt_ecal = ( tbragg_filter + rand()/(1.0*RAND_MAX) ) * 10;
          ptr->alt_ecal = tbragg_filter;
        }
        // printf("%d TBragg filter = %f with max = %f, ",ptr->ts,tbragg_filter,tbragg_filter_max);
        // printf("sum1 = %f, sum2 = %f\n",tbragg_sum1,tbragg_sum2);
      }
    }
    break;
