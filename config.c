...
int write_config(Config *cfg, FILE *fp)
{
   ...
   for(i=0; i<cfg->ncal; i++){ calib = cfg->calib[i];
      ...
      }else if(strncmp(calib->name,"GRG",3)==0){
...
int clear_calibrations(Config *cfg)
{
  ...
  for(i=0; i<cfg->ncal;     i++){
    edit_calibration(cfg, cfg->calib[i]->name, offset, gain, quad, puk1, puk2, puE1, ct0, ct1, ct2, address, datatype, 1);
  }
  return(0);
}
