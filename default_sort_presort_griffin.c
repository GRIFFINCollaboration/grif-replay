...
int pre_sort_enter(int start_idx, int frag_idx)
{
  ...
  float energy, ecal, inv_integ, psd, correction;
  int i, ppg_index, ptr_subsys;
  ...
  if (__builtin_expect(ptr->integ1 != 0, 1)) {
    inv_integ = 1.0/ptr->integ1;
    energy = spread(ptr->q1) * inv_integ;
  } else {
    energy = ptr->q1;
  }
  ...
  if((unsigned int)(energy - 1) < (unsigned int)(E_SPECLEN - 1)){
    batch_data_ph_hist[chan][(int)energy]++;
  }
  ...
  subsys_deadtime_count[ptr_subsys] += (ptr->deadtime>0) ? ptr->deadtime : subsys_prg_ddtm[ptr_subsys];
  if(ptr->trig_acc - previous_trig_acc[chan] != 1 && ptr->trig_acc - previous_trig_acc[chan] != 16383){
    if(ptr->trig_acc - previous_trig_acc[chan]>16100){ // Handle the wrap at 14 bits
      ...
      add *= (ptr->deadtime>0) ? ptr->deadtime : subsys_prg_ddtm[ptr_subsys];
      subsys_deadtime_count[ptr_subsys] += add;
  ...
  if(ppg_last_ptr_ts>ppg_pattern_end && ptr_ts>ppg_last_ptr_ts){
    ...
    if(ppg_cycle_step==ppg_cycle_length){ // Move to next cycle
      ppg_cycle_step = 0;
      fill_batch_cycles_histos();                                  // Trigger the batch filling for the ppg_cycle_number just finishing
  ...
  if( ptr_subsys == SUBSYS_TAC_LABR || ptr_subsys == SUBSYS_TAC_ZDS || ptr_subsys == SUBSYS_TAC_ART){
    ptr->ts -= tac_ts_offset[crystal_table[chan]-1]; // Subtract some amount from TAC timestamps
  }

  // DESCANT detectors
  // use psd for Pulse Shape Discrimination provides a distinction between neutron and gamma events
  //if( ptr_subsys == SUBSYS_DESCANT || ptr_subsys == SUBSYS_DESWALL){
  if( ptr_subsys == SUBSYS_DESWALL){
  ...
  if( ptr_subsys == SUBSYS_HPGE_A){
    ...
    while( i != frag_idx ){ // need at least two events in window
      ...
      if((dt=ptr_ts - alt->ts)>479 || alt->subsys != SUBSYS_HPGE_A){ continue; }
    ...
    while( i != frag_idx ){ // need at least two events in window
      ...
      if((dt=ptr_ts - alt->ts)>1000 || alt->subsys != SUBSYS_HPGE_A){ continue; }
        ...
        if(chan2 != chan ){
          if((clover=(int)(crystal_table[chan2]>>2)) == (int)(crystal_table[chan]>>2)){
            ...
            switch(crystal_table[chan2]&3){
              case 0:  ct_e_vs_dt_B[crystal_table[chan]]->Fill(ct_e_vs_dt_B[crystal_table[chan]], ((int)((alt->ts - ptr_ts)>>2)+300), (int)ptr->ecal-1100, 1); break;
              case 1:  ct_e_vs_dt_G[crystal_table[chan]]->Fill(ct_e_vs_dt_G[crystal_table[chan]], ((int)((alt->ts - ptr_ts)>>2)+300), (int)ptr->ecal-1100, 1); break;
              case 2:  ct_e_vs_dt_R[crystal_table[chan]]->Fill(ct_e_vs_dt_R[crystal_table[chan]], ((int)((alt->ts - ptr_ts)>>2)+300), (int)ptr->ecal-1100, 1); break;
              case 3:  ct_e_vs_dt_W[crystal_table[chan]]->Fill(ct_e_vs_dt_W[crystal_table[chan]], ((int)((alt->ts - ptr_ts)>>2)+300), (int)ptr->ecal-1100, 1); break;
            }
          }
        }
      }
  ...
  } // end of if( ptr_subsys == SUBSYS_HPGE_A){
    return(0);
  }
  ...
  int pre_sort_exit(int frag_idx, int end_idx)
  {
    ...
    int chan,chan2,found,pos, ptr_subsys, alt_subsys;
    ...
    while( i != end_idx ){ // need at least two events in window
      ...
      alt_subsys = alt->subsys;
      if(alt_subsys != SUBSYS_HPGE_A && dt>250){ continue; }

      // Determine multiplicity
      ptr_subsys = ptr->subsys;
      if( alt_subsys == ptr_subsys ){
        if(ptr_subsys == SUBSYS_HPGE_A){
        ...
        }else if(ptr_subsys == SUBSYS_QED_STRIP){
      ...
      static const void* const fill_presort_dispatch_table[MAX_SUBSYS] = {
        [ 0] =  &&do_presort_HPGE_A,
        [ 1] =  &&do_presort_PACES,
        [ 2] =  &&do_presort_LABR_L,
        [ 3] =  &&do_presort_RCMP,
        [ 4] =  &&do_presort_ARIES_A,
        [ 5] =  &&do_presort_ZDS_A,
        [ 6] =  &&do_presort_default, //&&do_presort_TAC_LABR,
        [ 7] =  &&do_presort_default, //&&do_presort_LABR_BGO,
        [ 8] =  &&do_presort_default, //&&do_presort_BGO,
        [ 9] =  &&do_presort_SCEPTAR,
        [10] =  &&do_presort_default, //&&do_presort_DESCANT,
        [11] =  &&do_presort_default, //&&do_presort_DESWALL,
        [12] =  &&do_presort_default, //&&do_presort_DSG,
        [13] =  &&do_presort_QED_STRIP,
        [14] =  &&do_presort_default, //&&do_presort_IGNORE,
        [15] =  &&do_presort_default, //&&do_presort_DCOMPTONA,
        [16] =  &&do_presort_HPGE_B,
        [17] =  &&do_presort_default, //&&do_presort_ARIES_B,
        [18] =  &&do_presort_ZDS_B,
        [19] =  &&do_presort_TAC_ZDS,
        [20] =  &&do_presort_TAC_ART,
        [21] =  &&do_presort_default, //&&do_presort_COMPTONA,
        [22] =  &&do_presort_QED_PIXEL,
        [23] =  &&do_presort_default, //&&do_presort_DCOMPTONB,
        [24] =  &&do_presort_default, //&&do_presort_COMPTONB,
        [25] =  &&do_presort_default, //&&do_presort_IGNORE4,
        [26] =  &&do_presort_default, //&&do_presort_IGNORE5,
        [27] =  &&do_presort_default  //&&do_presort_UNKNOWN
      };
      ...
      goto *fill_presort_dispatch_table[ptr_subsys];


      // SubSystem-specific pre-processing
      do_presort_HPGE_A:
      // HPGe Clover time-dependant crosstalk corrections
      if(alt_subsys == SUBSYS_HPGE_A && chan2 != chan){
        if((clover=(int)(crystal_table[chan2]>>2)) == (int)(crystal_table[chan]>>2)){
      ...
      if( (dt >= addback_window_min && dt <= addback_window_max) && alt_subsys == SUBSYS_HPGE_A ){
      ...
      if( alt_subsys == SUBSYS_QED_STRIP ){
      ...
      if( alt_subsys == SUBSYS_SCEPTAR && (dt >= time_diff_gate_min[SUBSYS_HPGE_A][SUBSYS_SCEPTAR]) && (dt <= time_diff_gate_max[SUBSYS_HPGE_A][SUBSYS_SCEPTAR]) ){ ptr->tof = ptr->tof | 1; break; }
      if( alt_subsys == SUBSYS_ZDS_A   && (dt >= time_diff_gate_min[SUBSYS_HPGE_A][SUBSYS_ZDS_A])   && (dt <= time_diff_gate_max[SUBSYS_HPGE_A][SUBSYS_ZDS_A])   ){ ptr->tof = ptr->tof | 2; break; }
      if( alt_subsys == SUBSYS_ARIES_A && (dt >= time_diff_gate_min[SUBSYS_HPGE_A][SUBSYS_ARIES_A]) && (dt <= time_diff_gate_max[SUBSYS_HPGE_A][SUBSYS_ARIES_A]) ){
        ptr->tof = ptr->tof | 4;
      ...
      goto presort_loop_end;
      do_presort_SCEPTAR:
      if(alt_subsys == SUBSYS_HPGE_A && (dt >= time_diff_gate_min[SUBSYS_HPGE_A][SUBSYS_SCEPTAR]) && (dt <= time_diff_gate_max[SUBSYS_HPGE_A][SUBSYS_SCEPTAR]) ){ alt->tof = alt->tof | 1; }
      if(alt_subsys == SUBSYS_PACES  && (dt >= time_diff_gate_min[SUBSYS_PACES][SUBSYS_SCEPTAR])  && (dt <= time_diff_gate_max[SUBSYS_PACES][SUBSYS_SCEPTAR])  ){ alt->tof = alt->tof | 1; }
      goto presort_loop_end;
      do_presort_ZDS_A:
      if(alt_subsys == SUBSYS_HPGE_A && (dt >= time_diff_gate_min[SUBSYS_HPGE_A][SUBSYS_ZDS_A])   && (dt <= time_diff_gate_max[SUBSYS_HPGE_A][SUBSYS_ZDS_A])   ){ alt->tof = alt->tof | 2; }
      if(alt_subsys == SUBSYS_PACES  && (dt >= time_diff_gate_min[SUBSYS_PACES][SUBSYS_ZDS_A])    && (dt <= time_diff_gate_max[SUBSYS_PACES][SUBSYS_ZDS_A])    ){ ptr->tof = alt->tof | 2; }
      goto presort_loop_end;
      do_presort_ARIES_A:
      if(alt_subsys == SUBSYS_PACES  && (dt >= time_diff_gate_min[SUBSYS_PACES][SUBSYS_ARIES_A])  && (dt <= time_diff_gate_max[SUBSYS_PACES][SUBSYS_ARIES_A])  ){ alt->tof = alt->tof | 4; }
      if(alt_subsys == SUBSYS_HPGE_A && (dt >= time_diff_gate_min[SUBSYS_HPGE_A][SUBSYS_ARIES_A]) && (dt <= time_diff_gate_max[SUBSYS_HPGE_A][SUBSYS_ARIES_A]) ){
        alt->tof = alt->tof | 4;
        pos = crystal_table[alt->chan];
        if((pos>12 && pos<17) || (pos>64 && pos<69)){
          // Triangles
      ...
      if(alt_subsys == SUBSYS_HPGE_B && chan2 == chan){
      ...
      // ptr->tof = 4 = Beta coincidence with ARIES only
      // The bit assignments are cumulative eg. pt->tof = 5 = Beta coincidence with both SCEPTAR and ARIES.
      if( alt_subsys == SUBSYS_SCEPTAR && (dt >= time_diff_gate_min[SUBSYS_PACES][SUBSYS_SCEPTAR]) && (dt <= time_diff_gate_max[SUBSYS_PACES][SUBSYS_SCEPTAR]) ){ ptr->tof = ptr->tof | 1; goto presort_loop_end; }
      if( alt_subsys == SUBSYS_ZDS_A   && (dt >= time_diff_gate_min[SUBSYS_PACES][SUBSYS_ZDS_A])   && (dt <= time_diff_gate_max[SUBSYS_PACES][SUBSYS_ZDS_A])   ){ ptr->tof = ptr->tof | 2; goto presort_loop_end; }
      if( alt_subsys == SUBSYS_ARIES_A && (dt >= time_diff_gate_min[SUBSYS_PACES][SUBSYS_ARIES_A]) && (dt <= time_diff_gate_max[SUBSYS_PACES][SUBSYS_ARIES_A]) ){ ptr->tof = ptr->tof | 4; goto presort_loop_end; }
      goto presort_loop_end;
      do_presort_RCMP:
      ...
      if( alt_subsys == SUBSYS_RCMP && (dt >= rcmp_fb_window_min && dt <= rcmp_fb_window_max) && (ptr->ecal>0 && ptr->ecal<32768)){
      ...
      // The charged particles enter the P side and this has superior energy resolution
      // Ensure the energy collected in the front and back is similar
      if( alt_subsys == SUBSYS_QED_STRIP && (dt >= qed_fb_window_min && dt <= qed_fb_window_max) && (ptr->ecal>QED_STRIP_THRESHOLD && ptr->ecal<32768)){
      ...
      if( alt_subsys == SUBSYS_QED_STRIP && (dt >= qed_fb_window_min && dt <= qed_fb_window_max) && (alt->ecal>QED_STRIP_THRESHOLD && alt->ecal<32768)){
      ...
      if( alt_subsys == SUBSYS_TAC_LABR ){
        ...
        if( dt >= lbl_tac_window_min && dt <= lbl_tac_window_max ){
          if( alt->q2 < 1 ){ // First LBL in coincidence with this TAC
            ...
            if( alt->integ2 < 0 || alt->integ2 >= odb_daqsize ){ goto presort_loop_end; }
  ...
  int pre_sort_triples(int frag_idx, int end_idx)
  {
    ...
    // This presort is only used for QED analysis
    if( subsys_initialized[SUBSYS_QED_STRIP] == 0 ){
      return(0);
    }
    ...
    while( i != end_idx ){ // need at least two events in window
      ...
      // Here can look to reconstruct SUBSYS_COMPTONB events, Si-Si scatters of 511keV
      if(triples_multiplicity[TRIPLES_QED] == 2){

        if((ptr->ecal+alt->ecal)>495 && (ptr->ecal+alt->ecal)<527){
          ...
          if(pos1!=pos2){
            ...
            if((angle1>=compton_angle(alt->ecal,QED_GAMMA_ENERGY)-QED_ANGLE_WINDOW) && (angle1<=compton_angle(alt->ecal,QED_GAMMA_ENERGY)+QED_ANGLE_WINDOW)){
              //  fprintf(stdout,"================Triples presort COMPTONB IDENTIFIED %d: %d %d, %d %d, %.1f %.1f (%.1f), %d\n",frag_idx,ptr->subsys,alt->subsys,pos1,pos2,ptr->ecal,alt->ecal,(ptr->ecal+alt->ecal),dt);
              ...
              ptr->subsys = SUBSYS_COMPTONB;                  // First Si hit becomes COMPTONB event, Si-Si scatter
              ptr->alt_ecal  = alt->ecal;                     // Second QED pixel energy
              ptr->esum = ptr->ecal + alt->ecal;              // esum will be the full energy (511keV)
              ...
              ptr->delta_t = alt->ts-ptr->ts;                 // Time difference of the two Hits

              triples_multiplicity[TRIPLES_QED]--; // Subtract this former pixel from the multiplicity
              continue;
            }
            angle1 = scattering_angle_QEDQED(pos2, qed2, pos1, qed1);
            if((angle1>=compton_angle(ptr->ecal,QED_GAMMA_ENERGY)-QED_ANGLE_WINDOW) && (angle1<=compton_angle(ptr->ecal,QED_GAMMA_ENERGY)+QED_ANGLE_WINDOW)){
              //  fprintf(stdout,"================Triples presort COMPTONB IDENTIFIED %d: %d %d, %d %d, %.1f %.1f (%.1f), %d\n",frag_idx,ptr->subsys,alt->subsys,pos1,pos2,ptr->ecal,alt->ecal,(ptr->ecal+alt->ecal),dt);

              // In COMPTONB event, ecal will be 1st QED_PIXEL and alt_ecal will be 2nd QED_PIXEL, esum will be the full energy
              // In COMPTONB event, crystal_table[chan]=pos will be  first QED DSSD number [1-6],  alt_chan will be [DSSD*PIXELnumber],[0-5*0-1023]
              // In COMPTONB event, crystal_table[ tof]=pos will be second QED DSSD number [1-6], alt2_chan will be [DSSD*PIXELnumber],[0-5*0-1023]
              alt->subsys = SUBSYS_COMPTONB;                  // First Si hit becomes COMPTONB event, Si-Si scatter
              alt->alt_ecal  = ptr->ecal;                     // Second QED pixel energy
              alt->esum = ptr->ecal + alt->ecal;              // esum will be the full energy (511keV)
              ...
              alt->delta_t = ptr->ts-alt->ts;                 // Time difference of the two Hits
      ...
      // Need 3 hits for triple coincidences
  ...
  if(ptr->subsys != SUBSYS_COMPTONA){ return(0); }
...
// Only interested in COMPTON coincidences in this presort
if(alt->subsys != SUBSYS_COMPTONA){ continue; }
//  fprintf(stdout,"Accepted Doubles: %d %d: %d: %d %d, %.1f %.1f (%.1f)\n",frag_idx,i,(ptr->ts - alt->ts),ptr->subsys,alt->subsys,ptr->ecal,alt->ecal,(ptr->ecal+alt->ecal));
...
azimuthal = (int)azimuthal_DCS_SiGe_SiGe(pos1, qed1, ge1, pos2, qed2, ge2);
