...
int process_event(Grif_event *ptr, int slot)
{
  ...
  static time_t prv_time, cur_time;

  // Calling time(NULL) every event was consuming 98% of the execution time for this function,
  // Which was about 25% of total execution time
  // Only fetch real time every 1000 calls to avoid system overhead
  if ((calls & 0x3FF) == 0 || prv_time == 0) {
    cur_time = time(NULL); // This is a slow system call
  } else {
    // Increment internal second counter loosely if needed,
    // or just let cur_time stay fixed until the 1000-call block refreshes it
    // A safer way for exact 10-sec logic is checking time more frequently
  ...
  int de = calls - prv_call;
  prv_call = ++calls;

  // Check actual time less frequently, or grab time only when counter reaches a threshold
  if (calls - prv_call > 5000 || (cur_time - prv_time >= 10)) {
    cur_time = time(NULL); // Real update when due for check
  }

  int dt = cur_time - prv_time;
