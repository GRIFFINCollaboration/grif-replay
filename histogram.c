...
TH1I *H1_BOOK(Config *cfg, char *name, char *title, int nbins, int xmin, int xmax)
{
  ...
  {
    // Fast path: Pure integer math assuming xmin == 0 and xbins == xrange
    int bin = xval;

    // Slow path calculation (using fixed-point or pure integer math if possible)
    // keep floating-point inside the unlikely block
    if (__builtin_expect(this->xmin != 0 || this->xbins != this->xrange, 0)) {
      bin = (int)(((float)xval - (float)this->xmin) * ((float)this->xbins / (float)this->xrange));
    }

    // Combined bounds check: casting to unsigned checks both 'bin < 0' and 'bin >= xbins' in one instruction
    if (__builtin_expect((unsigned int)bin >= (unsigned int)this->xbins, 0)) {
      if (bin < 0) {
        this->underflow++;
      } else {
        this->overflow++;
  ...
  int TH1I_Batch_Fill(TH1I *this, int *batch_data)
  {
    int *data = this->data;
  ...
  {
    // Fast path: map values to bins using integer math
    int ix = (int)(((long long)(xval - this->xmin) * this->xbins) / this->xrange);
    int iy = (int)(((long long)(yval - this->ymin) * this->ybins) / this->yrange);

    if (__builtin_expect(ix < 0 || ix >= this->xbins, 0)) {
      if (ix < 0) this->underflow++; else this->overflow++;
      return 0;
    }
    if (__builtin_expect(iy < 0 || iy >= this->ybins, 0)) {
      if (iy < 0) this->underflow++; else this->overflow++;
      return 0;
    }
    ...
    int *data = this->data;
    if (data == NULL) {
      size_t total_bins = (size_t)this->xbins * (size_t)this->ybins;
    ...
    this->data[ix + iy * this->xbins] += count;
    return 0;
  }
