...
void reorder_main(Sort_status *arg)
{
  ...
  int i,  grifc,  type,  dtype, rd_avail,  ts_stat,  err_format, scaler_event;
  ...
  while(1){
    ...
    // now have full event
    if( scaler_event ){
      evstart = NULL; err_format = len = ev_done = ts_stat = scaler_event = 0; continue;
    }
    if( err_format ){
    ...
    if( ts > (output_ts + TOO_EARLY_CUTOFF) ){
      /*fprintf(stdout,"ERR_EARLY_IN: ts > (output_ts + TOO_EARLY_CUTOFF), %ld > (%ld + %ld [%ld])\n",ts,output_ts,TOO_EARLY_CUTOFF,(output_ts + TOO_EARLY_CUTOFF));*/
